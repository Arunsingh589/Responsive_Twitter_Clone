...
import { FaAdversal, FaBookmark, FaChartLine, FaChevronDown, FaHashtag, FaThumbsUp, FaShare, FaComment, FaHome, FaList, FaRegEnvelope, FaSearch, FaSignOutAlt, FaUser, FaPlus, FaTimes, FaImage, FaLeaf, FaSmile, FaCamera, FaChartBar, FaRetweet, FaCheckCircle, FaMoneyCheck, FaBriefcase } from 'react-icons/fa';
...
const Main2 = () => {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark' : ''}`} style={{ backgroundColor: darkMode ? '#10171e' : '#d6e1e9' }}>
      ...
      {/* Nav section */}
      <nav className="fixed w-full h-20 bg-white dark:bg-white flex justify-center shadow-sm top-0 left-0 z-10">
        <div className="flex items-center">
          <div className="flex items-center mr-8">
            <FaHome className="text-3xl text-gray-400 dark:text-gray-300  mr-[5rem] hover:text-[#1aa1f5]" />
           <Link to={'/grok'}> <GoSkipFill className="text-3xl text-gray-400 dark:text-gray-300 mr-[5rem] hover:text-[#1aa1f5]" /></Link>
           <Link to={'/notification'}> <FaBell className="text-3xl text-gray-400 mr-[5rem] dark:text-gray-300 hover:text-[#1aa1f5]" /></Link>
            <Link to={'/message'} ><FaRegEnvelope className="text-3xl text-gray-400 dark:text-gray-300 mr-[5rem] hover:text-[#1aa1f5]" /></Link>
          </div>
          <div className="relative flex items-center">
            <i className="absolute text-xl left-4 text-[#9a9a9a] dark:text-gray-300"><FaSearch /></i>
            <input type="text" placeholder='Search X' className="w-80 h-10 border dark:text-gray-300 text-[#9a9a9a] dark:bg-gray-800 rounded-full bg-[#d6e1e9] pl-12 transition-all dark:focus:bg-gray-600 focus:bg-white focus:border-[#1aa1f5]" />
          </div>
          <div className="ml-12 flex items-center cursor-pointer relative">
            <div className="w-12 h-12">
              <img src="user1.jpg" alt="User" className="w-full h-full object-cover rounded-full" />
            </div>
            <p className="ml-4 text-lg text-gray-800 dark:text-gray-300">Arun Singh</p>
            <div onClick={toggleMenu} className="ml-2 text-xl text-gray-800 cursor-pointer dark:text-gray-300">
              <FaChevronDown />
            </div>
          </div>
        </div>
      ...
      {/* Post section */}
      <main className="mt-20 p-4 flex justify-center" style={{ backgroundColor: darkMode ? '#10171e' : '#d6e1e9' }}>
        <div className="flex w-full max-w-screen-xl">
          <div className="w-2/3 pr-8">
            <div className="fixed bottom-8 right-36">
              <button className="bg-[#1aa1f5] text-white px-8 font-semibold text-2xl flex items-center py-5 rounded-full hover:bg-blue-600" onClick={() => setShowModal(true)}>+<FaLeaf /> Post</button>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-grow">
                  <input type="text" placeholder="What's happening?" className="w-full h-10 border dark:text-gray-300 text-[#9a9a9a] dark:bg-gray-800 bg-[#d6e1e9] pl-4 rounded-full transition-all dark:focus:bg-gray-600 focus:bg-white focus:border-[#1aa1f5]" />
                </div>
                <div className="flex items-center gap-4">
      ...
      {menuVisible && (
        <>
          ...
          <div ref={menuRef} className="fixed right-0 top-0 h-full w-1/4 dark:bg-gray-800 bg-white border-l shadow-sm transition-right duration-500 ease-in-out z-20">
